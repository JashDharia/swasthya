...
echo "<br>";
    if(isset($_SESSION['user_id'])||isset($_SESSION['profile_id'])){
        ...
        echo "<button name='logout' style=''>Log out</button>";
        echo "</form>";
        echo "</div>";
      }
    }
...
if(!isset($_GET['profile'])&&!(isset($_GET['id']))){
  ...
  $user_id=$_COOKIE['user_id'];
    ...
    $conn=new mysqli("localhost","root","","swasthya");
      ...
      $sql="SELECT url FROM posts WHERE profile_url='$user_id'";
      ...
      if ($r===NULL) {
        ...
        echo "<img src='http://localhost/swasthya/dp/dp/".$r['url']."' class='img-fluid'></img>";
      }
      echo "</div>";
      if(isset($_SESSION['profile_id'])&&!isset($_GET['id'])||!isset($_GET['profile']))
      echo "<button class='edit'>edit dp</button>";
      echo "</div>";
      echo "<br>";
    ?>
    <script type="text/javascript">
      $(document).ready(function(){
        $(".edit").on("click",function(){
          window.location.href="http://localhost/swasthya/dp/";
        });
      });
    </script>
    <div style="padding: 10px;">
      ...
      ?>  
        ...
        <tr>
          ...
          <td>
          <?php
      echo ":".$row['qualification1'].",".$row['qualification2'].",".$row['qualification3'];
          ?>
          </td>
        </tr>
        <tr>
...
<script type="text/javascript">
  $(".book_appointment").ready(function(){
    $(".book_appointment").on("click",function(){
      <?php if(isset($_SESSION['profile_id'])){echo "window.location.href='http://localhost/swasthya/booking/'";}else{echo "alert('please login to book appointment');window.location.href='http://localhost/swasthya/signup/'";}?>
    });
  });
</script>
<br>
...
<br>
  <?php
  if (isset($_SESSION['profile_id'])&&$_SESSION['user_type']!='user') {
    $res=getClinics($_SESSION["profile_id"]);
    }
    else{
      $res=getClinics($_GET['id']);
    }
    if($res!=NULL){
        ?>
...
<?php 
  ...
  ?>
    <tr class="tr">
      ...
      <?php
      if(isset($_SESSION['profile_id'])&&!isset($_GET['profile'])||!isset($_GET['id'])||!isset($user_id)){
      if($_SESSION['user_type']=='doctor'){
        echo "<td style='background-color:#e6f5ff;'>";
        echo "<button class='edit_".$row['id']."'>edit</button>";
        echo "<td>";
      }
      ?>
      <script type="text/javascript">
        $(document).ready(function(){
          $(".edit_<?php echo $row['id'] ?>").on("click",function(){
            window.location.href="http://localhost/swasthya/edit/?id=<?php echo $row['id'] ?>&user=<?php echo $_SESSION['profile_id']?>"
          });
        });
      </script>
    </tr>
...
<br>
<?php
if(isset($_SESSION['profile_id'])&&!isset($_GET['profile'])&&!isset($_GET['id'])) {
  ?>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      $(".patients").on("click",function(){
        window.location.href="https://swaasthya.co/patients/"
      });
    });
  </script>
...
<div style="text-align: left;color: white; background-color:#001a33;">
    ...
    <br>
  <div style="margin-right: 10px;" class="table">
